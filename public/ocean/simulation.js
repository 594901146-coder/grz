...
var Simulator = function (canvas, width, height) {
    ...
    var oceanRadius = 2400,
        oceanCenter = [0, -400],
        oceanRadialSegments = 127,
        oceanAngularSegments = 512,
        oceanData = [],
        oceanIndices = [];

    oceanData.push(oceanCenter[0]);
    oceanData.push(0.0);
    oceanData.push(oceanCenter[1]);
    oceanData.push((oceanCenter[0] - GEOMETRY_ORIGIN[0]) / GEOMETRY_SIZE);
    oceanData.push((oceanCenter[1] - GEOMETRY_ORIGIN[1]) / GEOMETRY_SIZE);

    for (var ringIndex = 1; ringIndex <= oceanRadialSegments; ringIndex += 1) {
        var radius = oceanRadius * ringIndex / oceanRadialSegments;

        for (var segmentIndex = 0; segmentIndex < oceanAngularSegments; segmentIndex += 1) {
            var angle = Math.PI * 2 * segmentIndex / oceanAngularSegments,
                x = oceanCenter[0] + Math.cos(angle) * radius,
                z = oceanCenter[1] + Math.sin(angle) * radius;

            oceanData.push(x);
            oceanData.push(0.0);
            oceanData.push(z);
            oceanData.push((x - GEOMETRY_ORIGIN[0]) / GEOMETRY_SIZE);
            oceanData.push((z - GEOMETRY_ORIGIN[1]) / GEOMETRY_SIZE);
        }
    }

    for (var segmentIndex = 0; segmentIndex < oceanAngularSegments; segmentIndex += 1) {
        var current = 1 + segmentIndex,
            next = 1 + (segmentIndex + 1) % oceanAngularSegments;

        oceanIndices.push(0);
        oceanIndices.push(next);
        oceanIndices.push(current);
    }

    for (var ringIndex = 1; ringIndex < oceanRadialSegments; ringIndex += 1) {
        var innerStart = 1 + (ringIndex - 1) * oceanAngularSegments,
            outerStart = 1 + ringIndex * oceanAngularSegments;

        for (var segmentIndex = 0; segmentIndex < oceanAngularSegments; segmentIndex += 1) {
            var nextSegment = (segmentIndex + 1) % oceanAngularSegments,
                innerCurrent = innerStart + segmentIndex,
                innerNext = innerStart + nextSegment,
                outerCurrent = outerStart + segmentIndex,
                outerNext = outerStart + nextSegment;

            oceanIndices.push(innerCurrent);
            oceanIndices.push(outerNext);
            oceanIndices.push(outerCurrent);
            oceanIndices.push(innerCurrent);
            oceanIndices.push(innerNext);
            oceanIndices.push(outerNext);
        }
    }
